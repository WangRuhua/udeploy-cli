...
module.exports = function(argv) {
  ...
  .option("e", { alias: "environment", describe: "create a snapshot from envrionment", type: "string" })
  ...
  VERBOSE_LEVEL >=2 && console.log('this is snapshot',argv)

  const baseURL = options.baseURL || process.env.baseURL
  if (! baseURL ) {
    console.log("please assign a url to baseURL");
    process.exit(1);
  } else {
  ...
  if (! udeploy_token ) {
    console.log("please assign a token to udeploy_token");
    process.exit(1);
  }
  ...
  if (options.create) {
    q={application: options.application, name: options.snapshot, environment: options.environment}
    console.log("creating snapshot of ",q)
    createSnapshotOfEnvironment(q) 
  }
  ...
  function addSnapshotStatus(q) {
    return instance.put('cli/snapshot/addStatusToSnapshot','',{
      ...
      .catch(function (error) {
        console.log("error code:",error.response.status, "error data:",error.response.data)
        VERBOSE_LEVEL >=1 && console.log("error data:",error)
      });
    }

  function createSnapshotOfEnvironment(q) {
    instance.put('/cli/snapshot/createSnapshotOfEnvironment','',{ params: q } 
      )
      .then(function (response) {
        console.log(response.data);
      })
