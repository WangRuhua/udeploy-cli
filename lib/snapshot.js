...
// add new status
if (options.update) {
  q={application:options.application, snapshot:options.snapshot, statusName:options.update}
    ...
    .then(function (response) {
      ...
      if(response.data[0] === undefined) {
        ...
        for( var i=0;i<response.data.length;i++) {
          ...
          console.log("Status of",q.application,":",chalk.hex(obj.color)(obj.name))
        }
      }
    })
...
function addSnapshotStatus(q) {
  instance.put('cli/snapshot/addStatusToSnapshot','',{
    params: q})
    .then(function (response) {
